#####################################################
## Care Home Global Script ##
#####################################################


### Script loads data files needed to populate the Care Home shiny app and 
### sets up formatting for shiny app colour schemes and buttons
...
if (!require('htmltools')) install.packages('htmltools'); library('htmltools') # shiny package dependency
if (!require('rlang')) install.packages('rlang'); library('rlang') # bslib package dependency
if (!require('grDevices')) install.packages('grDevices'); library('grDevices') # bslib package dependency
if (!require('jsonlite')) install.packages('jsonlite'); library('jsonlite') # bslib package dependency
if (!require('sass')) install.packages('sass'); library('sass') # bslib package dependency
if (!require('jquerylib')) install.packages('jquerylib'); library('jquerylib') # bslib package dependency
if (!require('bslib')) install.packages('bslib'); library('bslib') # shiny package dependency
...
## Time Period object 
###########################
# this will be used in plot titles where time period is specified
# this will be the latest financial year the apps hold data for

data_time_period_start <- "2017/18"
data_time_period_end <- "2020/21"
first_quarter <- "2017/18 Q4"
last_quarter <- "2020/21 Q4"
first_census_week <- "2009"
last_census_week <- "2021"          

#####################
## Set File Paths & Load in data ----
############################.

# for shiny app to deploy, need to use relative file names based on working directory being the R project folder

data_filepath <- "data/"

## Read in all rds files ####

# read all .rds files from the shiny app data file
...
data_completeness_table <- readxl::read_excel(paste0(data_filepath, "data_completeness_table_care_home.xlsx"), col_names = TRUE)  %>% 
