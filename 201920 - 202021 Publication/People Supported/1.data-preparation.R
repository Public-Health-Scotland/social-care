...
### where real data files are not available yet, read in dummy files
...
data_people_supported_trend <- data_people_supported_trend %>% 
  ...
  mutate(financial_year   = as.character(financial_year),
         ...
         location_order   = ordered(dplyr::recode(sending_location, "Scotland" = "A",
         ...
         scotland_nclient = as.numeric(scot_nclient),
         scotland_rate    = as.numeric(scotland_rate)) %>% 
  arrange(financial_year, location_order,  sending_location, measure)  %>% 
  select(financial_year, sending_location, measure, nclient, rate, scotland_nclient, scotland_rate)
 ...
 data_people_supported_summary <- data_people_supported_summary %>% 
   ...
   mutate(financial_year   = as.character(financial_year),
          sending_location = dplyr::recode(sending_location, 
                                           ...
                                           "Perth & Kinross" = "Perth and Kinross"),
          sending_location = as.character(sending_location),
          location_order   = ordered(dplyr::recode(sending_location, 
                                                   "Scotland" = "A",
                                                   "Scotland (All Areas Submitted)" = "A",
                                                   "Scotland (Estimated)" = "A")),
          measure          = dplyr::recode(measure,
          ...
          scot_nclient     = as.numeric(scot_nclient)) %>% 
   arrange(financial_year, location_order,  sending_location, measure)  %>% 
   select(financial_year, sending_location, measure, rate, nclient, scot_nclient, scotland_rate)
...
data_people_supported_age_sex <- data_people_supported_age_sex %>% 
  ...
  mutate(financial_year   = as.character(financial_year),
         ...
         sex_order        = dplyr::recode(sex, "Female" = 1,
                                          "Male" = 2,
  ...
  select(financial_year, sending_location, age_group, sex, measure, nclient, rate)
...
data_people_supported_ethnicity <- 
  ...
  select(financial_year, sending_location, age_group, ethnicity, measure, nclient, rate)
...
data_people_supported_client_group <- data_people_supported_client_group %>% 
  ...
  select(financial_year, sending_location, age_group, client_group, measure, denominator, nclient, rate)
...
data_people_supported_services <- data_people_supported_services %>% 
   mutate(financial_year = as.character(financial_year),
          ...
          location_order    = dplyr::recode(sending_location, 
                                            ...
                                            "West Dunbartonshire"            = 32,
                                            "West Lothian"                   = 33),
         service_group    = dplyr::recode(service, 'HC' = "Home Care",
                                                          'HS' = "Housing Support",
                                                          'CH' = "Care Home",
                                                          'SW' = "Social Worker",
...
data_people_supported_meals <- data_people_supported_meals %>% 
  ...
  mutate(financial_year   = as.character(financial_year),
   ...
   select(financial_year, sending_location, age_group, measure, nclient, rate)
...
data_people_supported_living_alone <- 
  ...
  select(financial_year, sending_location, age_group, living_alone, measure, 
         nclient, percentage)
