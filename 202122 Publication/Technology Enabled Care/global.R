...
data_completeness_table <- readxl::read_excel(paste0(data_table_filepath, "tec_dc_dq.xlsx"),sheet = "DC", col_names = TRUE) %>% 
  arrange(`Health and Social Care Partnership`, `Data Set`, desc(`Financial Year`))

## DATA QUALITY TABLE -

data_quality_table <- readxl::read_excel(paste0(data_table_filepath, "tec_dc_dq.xlsx"),sheet = "DQ", col_names = TRUE) %>% 
  arrange(`Health and Social Care Partnership`, `Data Set`, desc(`Financial Year`))
...
# previously "f1"
plot_title_font <-  list(family = "Neue Helvetica",
                         size = 14,
                         color = '#ffffff')

# previously "f2"
plot_axis_font <- list(family = "Neue Helvetica",
                       size = 14,
                       color = '#ffffff')
...
buttons_to_remove <- list("select2d",
                          "lasso2d",
                          "zoomIn2d",
                          "zoomOut2d",
                          "autoScale2d",
